...
<div>
    <p align="center" style="background-color:#ccccff;color:black; padding: 10px; font-family: sans-serif; min-height: 35px; text-transform: uppercase; position: relative;">
        ...
        <?php if ($this->request->session()->read('Auth.User')): ?>
            ...
            <?= $this->Html->link('Logout', array('controller'=>'users', 'action'=>'logout'),['class'=>'btn btn-outline btn-primary','style'=>'position:absolute; right: 20px;']); ?>
        <?php else: ?>
            <?= $this->Html->link( 'Login', array('controller'=>'users', 'action'=>'login'),['class'=>'btn btn-outline btn-primary','style'=>'position:absolute; right: 20px;']); ?>
        <?php endif; ?>
    </p>
</div>
    ...
    <div class="container col-sm-12">
        ...
        <div class="collapse navbar-collapse" id="top-navbar-1">
            <ul class="nav navbar-nav navbar-right">
                ...
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-delay="1000">
                        <?= $this->Html->tag('span','Admin Functions') ?>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-left" role="menu">
                        <li><?= $this->Html->link('Manage Accounts',['controller' => 'users', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Manage Bookings',['controller' => 'bookings', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Manage Contents',['controller' => 'content', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Manage Staff',['controller' => 'staffmembers', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Slider Picture',['controller' => 'sliders', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Manage Services',['controller' => 'categories', 'action'=>'index']) ?></li>
                        ...
                        <li><?= $this->Html->link('Manage Calendars',['controller' => 'calendars', 'action'=>'index']) ?></li>
                        <li><?= $this->Html->link('Email Layout',['controller' => 'emails', 'action'=>'index']) ?></li>
                    </ul>
                </li>
            </ul>
        </div>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-sm-7 footer-copyright wow fadeIn">
                <?php
                $year = date("Y");
                echo "Vision Overseas " . $year . " - All rights reserved";
                ?>
            </div>
